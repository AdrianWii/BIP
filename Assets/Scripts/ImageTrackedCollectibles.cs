...
public class ImageTrackedCollectibles : MonoBehaviour
{
    ...
    private void UpdateTrackedImage(ARTrackedImage trackedImage)
    {
        ...
        {
            status = $"Saw '{imageName}', but no collectible config exists.";
            return;
        }
        ...
        if (playerStats != null && playerStats.IsCollected(config.collectibleId))
        {
            ...
            status = $"'{config.collectibleId}' already collected.";
            return;
        }
        ...
        {
            HideObject(trackedImage.trackableId);
            status = $"Saw '{imageName}', state: {trackedImage.trackingState}.";
            return;
        }
        ...
        status = $"Tracking '{imageName}'. Tap collectible.";
    }
    ...
    private void TryCollectAtScreenPosition(Vector2 screenPosition)
    {
        ...
        if (!collected)
        {
            status = $"'{collectible.CollectibleId}' was already collected.";
            collectible.gameObject.SetActive(false);
            return;
        }

        collectible.gameObject.SetActive(false);
        status = $"Collected '{collectible.CollectibleId}' (+{collectible.Points}).";
    }
